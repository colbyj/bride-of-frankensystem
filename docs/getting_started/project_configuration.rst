...
The secret key should be a random string unique to your project. Generate one with Python:
...
A common split is a ``config.toml`` with the development settings (SQLite, simple admin password, full ``PAGE_LIST``) and a ``production.toml`` that overrides the database URI, secret key, admin password, and external-ID settings.
...
* **Trivial secret key** — generate a real random string with the Python snippet earlier in this page rather than typing ``"abc123"``.
